...
export default function ExpensesTab() {
  ...
  const { expensesQuery, dateRange } = useTransactionQueries();
  const { data: transactions, isLoading, error, refetch } = expensesQuery;

  const handleOpenForm = useCallback(() => {
    openTransactionSheet("EXPENSE", ["expenses", "transactions", dateRange.startDate.toISOString().split('T')[0], dateRange.endDate.toISOString().split('T')[0]]);
  ...
  const totalExpenses = transactions?.reduce((sum, transaction) => sum + transaction.amountInCents, 0) || 0;
